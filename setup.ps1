...
function Test-Prerequisites {
    ...
    foreach ($location in $chromeLocations) {
        if (Test-Path $location) {
            Write-ColorOutput "✓ Google Chrome found at: $location" -Color "Success"
            $chromeFound = $true
            break
        }
    }
    ...
    if (-not $chromeFound) {
        Write-ColorOutput "⚠ Google Chrome not found. Please install Chrome to test the extension." -Color "Warning"
    }
    ...
    foreach ($file in $requiredFiles) {
        ...
        if (Test-Path $filePath) {
            ...
            Write-ColorOutput "✓ $file ($fileSize bytes)" -Color "Success"
        } else {
...
function Start-DevelopmentSetup {
    ...
    try {
        ...
        Write-ColorOutput "✓ Manifest name: $($manifest.name)" -Color "Success"
        Write-ColorOutput "✓ Manifest version: $($manifest.version)" -Color "Success"
        Write-ColorOutput "✓ Manifest version: $($manifest.manifest_version)" -Color "Success"
        
        # Check permissions
        if ($manifest.permissions -contains "activeTab" -and $manifest.permissions -contains "storage") {
            Write-ColorOutput "✓ Required permissions are present" -Color "Success"
        } else {
            Write-ColorOutput "⚠ Check permissions in manifest.json" -Color "Warning"
        }
        
    } catch {
        Write-ColorOutput "✗ Error validating manifest.json: $($_.Exception.Message)" -Color "Error"
        exit 1
    }
...
function Start-ExtensionTests {
    ...
    foreach ($file in $fileTests.Keys) {
        try {
            ...
            if ($result) {
                Write-ColorOutput "  ✓ $file: Passed" -Color "Success"
                $testResults += [PSCustomObject]@{ Test = "$file integrity"; Result = "PASS"; Time = (Get-Date) }
            } else {
                Write-ColorOutput "  ✗ $file: Failed" -Color "Error"
                $testResults += [PSCustomObject]@{ Test = "$file integrity"; Result = "FAIL"; Time = (Get-Date) }
            }
        } catch {
            Write-ColorOutput "  ✗ $file: Error - $($_.Exception.Message)" -Color "Error"
    ...
    try {
        ...
        if ($braceCount -eq $closeBraceCount) {
            Write-ColorOutput "  ✓ CSS: Balanced braces ($braceCount pairs)" -Color "Success"
            $testResults += [PSCustomObject]@{ Test = "CSS syntax"; Result = "PASS"; Time = (Get-Date) }
        } else {
            Write-ColorOutput "  ✗ CSS: Unbalanced braces (open: $braceCount, close: $closeBraceCount)" -Color "Error"
            $testResults += [PSCustomObject]@{ Test = "CSS syntax"; Result = "FAIL"; Time = (Get-Date) }
        }
    } catch {
        Write-ColorOutput "  ✗ CSS: Error checking syntax" -Color "Error"
    ...
    foreach ($jsFile in $jsFiles) {
        try {
            ...
            if ($parenCount -eq $closeParenCount -and $braceCount -eq $closeBraceCount) {
                Write-ColorOutput "  ✓ $jsFile: Basic syntax OK" -Color "Success"
                $testResults += [PSCustomObject]@{ Test = "$jsFile syntax"; Result = "PASS"; Time = (Get-Date) }
            } else {
                Write-ColorOutput "  ✗ $jsFile: Potential syntax issues" -Color "Error"
                $testResults += [PSCustomObject]@{ Test = "$jsFile syntax"; Result = "FAIL"; Time = (Get-Date) }
            }
        } catch {
            Write-ColorOutput "  ✗ $jsFile: Error checking syntax" -Color "Error"
    ...
    try {
        ...
        foreach ($file in $filesToPackage) {
            if (Test-Path $file) {
                ...
                Write-ColorOutput "  ✓ Added: $file" -Color "Success"
            } else {
                Write-ColorOutput "  ⚠ Skipped: $file (not found)" -Color "Warning"
            }
        }
        ...
        if (Get-Command Compress-Archive -ErrorAction SilentlyContinue) {
            ...
            Write-ColorOutput "✓ Package created: $packagePath" -Color "Success"
            ...
            Write-ColorOutput "✗ Compress-Archive not available. Cannot create ZIP package." -Color "Error"
        }
...
function Clear-DevelopmentFiles {
    ...
    foreach ($dir in $cleanupDirs) {
        if (Test-Path $dir) {
            ...
            Write-ColorOutput "✓ Removed: $dir" -Color "Success"
        }
    }
    ...
    foreach ($file in $tempFiles) {
        ...
        Write-ColorOutput "✓ Removed temp file: $($file.Name)" -Color "Success"
    }
...
function Show-Usage {
    ...
    Write-ColorOutput ""
    Write-ColorOutput "Usage: .\setup.ps1 [-Action <action>]" -Color "Info"
    Write-ColorOutput ""
    ...
    Write-ColorOutput "  .\setup.ps1                # Setup development environment" -Color "Info"
    Write-ColorOutput "  .\setup.ps1 -Action test   # Run tests" -Color "Info"
    Write-ColorOutput "  .\setup.ps1 -Action package # Create package" -Color "Info"
}
